...
export default function Text({title, password, id, value, placeholder, color, className, innerClass, outerClass, onChange}) {
    ...
    return (
        <div className={`text-cont ${className || ""} ${outerClass || ""}`}>
            ...
            <input type={(password?"password":"text")} id={formId} name={formId} value={formValue} placeholder={placeholder} className={`text-input ${innerClass} ${BORDERCOLORS[color] || BORDERCOLORS.default}`} onChange={ChangeValue} />
        </div>
    );
}